...
const ProductTemplate = ({ product }) => {
  ...
  const AddToCartHandler = (product) => {
    ...
    const findProductInCart = copyUser.cart.findIndex(
  ...
  return (
    <div className="w-[85vw] max-w-[500px] md:w-[340px]  md:h-[500px] lg:w-[420px] lg:h-[585px] mb-8 py-4 flex justify-center items-center flex-col md:px-2 ">
      <div className="relative w-fit h-full overflow-hidden group  ">
        ...
        <div className="absolute inset-0 flex items-end justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 z-10">
          {" "}
          {/* Added z-10 for button and overlay background */}
          ...
          <button
            ...
            className="absolute bottom-3 border-2 border-white text-sm px-6 py-2 bg-[#aa5607] text-white  opacity-0 md:translate-y-full md:group-hover:opacity-100 md:group-hover:translate-y-0 active:scale-[0.9] transition-all duration-300 z-20 cursor-pointer "
          >
            {" "}
            {/* Added z-20 for button to be above overlay */}
            Quick Add
          </button>
        </div>
      </div>
      ...
      <div className="w-full md:h-[200px]">
        ...
        <Link to={`/products/${product.id}`} className="cursor-pointer">
          <h2 className="text-lg mt-2 font-black md:h-[60px] w-[95%] md:text-xl">
            {product.title}
          </h2>
        </Link>
        ...
        <button
          ...
          className="md:hidden border-2 border-white text-sm px-6 py-2  mt-4 bg-[#aa5607] text-white active:scale-[0.9] transition-all duration-300 z-20 cursor-pointer "
        >
          {" "}
          {/* Added z-20 for button to be above overlay */}
          Quick Add
